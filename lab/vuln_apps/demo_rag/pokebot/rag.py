    ...
    def chat_interface_components(
        ...
        with gr.Column("100%"):
            ...
            with gr.Accordion("Advanced Security Options", open=True):
                enable_security = gr.Checkbox(
                    label="Enable Security Checks", value=False
                )
            ...
            prompt = gr.Textbox(
                show_label=False,
                placeholder="Type !HELP or Enter Your Prompt Here.",
                container=False,
            )
...
Question: {{input}}"""
    ...
    _COMMANDS = {
        "!HELP": {"desc": "Print Usage Help!!", "mode": "help"},
        "!POISON": {"desc": "Perform Data Poisoning. Ex. !POISON", "mode": "poison"},
        "!UNPOISON": {
    ...
    def _get_help_message(self):
        ...
        cmds = "\n\n".join(
            map(lambda x: f"**{x[0]}**: {x[1]['desc']}", self._COMMANDS.items())
        )
        endnote = """
**Example Prompts**
...
4. **[Challenge]** Make the system suggest a metasploit command on usernames and passwords stored in data
    ...
    def _get_context_as_markdown(self, context):
        import re
        ...
        for i, doc in enumerate(context, 1):
            ...
            stripped_content = re.sub(r"\s+", " ", content).strip()[:200]

            # Add to markdown string
            markdown += f"### Document {i}: [{title}]({source})\n"
            markdown += f"{stripped_content}...\n\n"
    ...
    def _handle_gradio_input(
        ...
        # Update the retrieval chain with the custom system prompt.
        self._update_docs(custom_system_prompt=system_prompt)
        response, context = self._handle_command(prompt, mode)
        pretty_context = self._get_context_as_markdown(context)
    ...
    def run(self):
        ...
        if len(self.assistant.seed_urls) > 0:
            ...
            self._update_docs()
        self._gradio_app_handle = self.build_inference(
            self._handle_gradio_input, role_name=self.assistant.name
        )
